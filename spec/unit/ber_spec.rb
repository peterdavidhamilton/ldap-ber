...
RSpec.describe BER do
  it '#config' do
    ...
    expect(BER.config[:pdu]).to have_key(25)
  end

  it '#lookup' do
    expect(BER.lookup(:pdu, 25)).to eql(:intermediate_response)
    expect(BER.lookup(:oid, '1.3.6.1.4.1.1466.115.121.1.2')).to eql('String')
  end

  it '#reverse_lookup' do
    expect(BER.reverse_lookup(:pdu, :intermediate_response)).to eql(25)
  end
